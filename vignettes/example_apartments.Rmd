...
library(gridExtra)
```
In this vignette we present an example of an application of `SAFE` package. It is based on `apartments` and `apartmentsTest` dataset from `DALEX` package.

First we fit two models to the original dataset - linear model (which is going to play the role of white-box model) and random forest (complex model that will serve us as a surrogate model).
...
We also create `explainer` objects for both models - we will use them later to create new variables and at the end to compare their performance.
...
Now, we create a `safe_extractor` object using `SAFE` package and surrogate model. Setting the argument verbose=FALSE stops progress bar from printing.
